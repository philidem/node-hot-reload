...
HotReloader.prototype.doReload = function(event) {
    ...
    for (var key in require.cache) {
        if (require.cache.hasOwnProperty(key)) {
            ...
            if (this._shouldUncacheModule(module.filename)) {
                ...
                // this.log('Uncached module: ' + module.filename);
            ...
            //this.log('Not uncaching ' + module.filename);
        }
    }

    if (uncacheCount !== 0) {
        this.log('Uncached ' + uncacheCount + ' ' + (uncacheCount === 1 ? 'module' : 'modules'));
    }


    for (var i = 0; i < modulesToReload.length; i++) {
        var moduleToReload = modulesToReload[i];
        this._reloadModule(moduleToReload);
    }
    ...
    function afterReload() {
        _this._waitFor.length = 0;
        _this.emit('afterReload', event);
        _this.log('Reload complete');
    }

    require('q').all(waitFor)
            .then(afterReload, afterReload);
};
